...
function Index({ location }) {
  ...
  const { brand, fetchBrand, tambahProduk, editProduk, API } = useContext(CMSContext);
              ...
              produkId
                ? <Grid item xs={10} className={classes.imgInput} >
                  <label for="file-input" style={{ cursor: 'pointer' }} >
                    ...
                    />
                    <p className={classes.imgTag} style={{ textAlign: 'center', marginTop: 0, marginBottom: 0 }}>Change foto</p>
                  </label>
            ...
            <Grid container spacing={5} alignItems="flex-start">
              ...
              <Grid item xs={10}>
                <TextField
                  ...
                  size="small"
                  fullWidth
                  helperText={<p style={{ textAlign: 'right', margin: 0 }}>{`${input.namaProduk?.length}/255`}</p>}
                  name="namaProduk"
        ...
        <Card className={classes.root} elevation={2}>
          <CardContent>
                ...
                >
                  {
                    input.TDS
                      ? (input.TDS.name
                        ? `${input.TDS.name.slice(0, 10)} ... ${input.TDS.name.slice(input.TDS.name.length - 8, input.TDS.name.length)}`
                        : `${input.TDS.slice(0, 10)} ... ${input.TDS.slice(input.TDS.length - 8, input.TDS.length)}`)
                      : ' + Tambah File TDS'
                  }
                </Button>
                <input
                ...
                >
                  {
                    input.MSDS
                      ? (input.MSDS.name
                        ? `${input.MSDS.name.slice(0, 10)} ... ${input.MSDS.name.slice(input.MSDS.name.length - 8, input.MSDS.name.length)}`
                        : `${input.MSDS.slice(0, 10)} ... ${input.MSDS.slice(input.MSDS.length - 8, input.MSDS.length)}`
                      )
                      : ' + Tambah File MSDS'
                  }
                </Button>
            ...
            <Grid container spacing={5} >
              <Grid item xs={2}>
                <Typography variant="body2" component="p" style={{ margin: '8px 0px' }}>
                  Sertifikasi
                </Typography>
              </Grid>
              <Grid item xs={10}>
                <Grid style={{ display: 'flex', flexDirection: 'column' }}>
                  {
                    sertifikasi.map((el, index) =>
                      <FieldSertifikasi key={'sertifikat', index} data={el} lengthData={sertifikasi.length} deleteSertifikasi={deleteSertifikasi} sertifikasiIndex={index} handleFile={handleFile} />
                    )
                  }
                </Grid>
                <p style={{ margin: '8px 0px', cursor: 'pointer' }} onClick={addSertifikasi}>+ Sertifikasi Baru</p>
              </Grid>
            </Grid>
          </CardContent>
        </Card>
        ...
        <Card className={classes.root} elevation={2}>
          <CardContent>
            ...
            <Grid container spacing={5}>
              ...
              <Grid item xs={10}>
                <FieldHargaGrosir handleSetHargaGrosir={handleSetHargaGrosir} data={newHargaGrosir} />
              </Grid>
            </Grid>
          </CardContent>
        </Card>
